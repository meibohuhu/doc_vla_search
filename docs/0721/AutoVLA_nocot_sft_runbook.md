...
> ⚠️ 若启用 fp32 master weights（**强烈建议**，见 [bf16_master_weights_bug.md](../0724/bf16_master_weights_bug.md)），
> DDP 下单卡会涨到 **~76GB**（fp32 参数 +12GB、AdamW 状态 +24GB）。
> A100-80G 可行但余量仅约 5GB；如遇 OOM：`batch_size` 降到 1、`accumulate_grad_batches` 翻倍以保持 global batch 不变。

---

## 5.5 action token 与轨迹：codebook 是怎么工作的

这一节回答三个反复出现的疑问：JSON 里明明只有轨迹，action token 从哪来？codebook 准不准？loss 到底在算什么？

### 5.5.1 JSON 里只有原始轨迹，action token 是运行时算的

预处理产出的 `<token>.json` **不含任何 action token**，只有原始位姿：

```json
"gt_trajectory": [[1.91, 0.01, -0.0], [3.72, -0.01, -0.0], ...]   // 10 个 (x, y, heading)
```

转换发生在 `SFTDataset.__getitem__` 里，每次取样本时现算：

```
gt_trajectory (10 个原始位姿)
  ↓ TrajectoryTargetBuilder.compute_targets()
  ↓ TokenProcessor._match_agent_token()      查 codebook 最近邻
gt_idx = [30, 226, 806, 386, 187, 187, 22, 74, 12, 87]     10 个索引
  ↓ ActionTokenizer(gt_idx)
"<action_30><action_226><action_806>..."                    文本
  ↓ 拼进 assistant 回复 → apply_chat_template → processor
input_ids / labels 的对应位置 = 151665 + 索引
```

**为什么不把 action token 存进 JSON**：codebook（`agent_vocab.pkl`）是固定的，存不存都能现算；存原始轨迹更可读，也不跟 codebook 版本耦合。

### 5.5.2 codebook 是双向的，量化误差约 5 cm

`codebook_cache/agent_vocab.pkl` 形状 **(2048, 6, 4, 2)** = 2048 个 token × 每个 6 小步运动基元 × 4 角点 × xy。

| 方向 | 用在哪 | 怎么做 |
|---|---|---|
| **encode** 轨迹 → token | **训练时造标签** | 每步把位姿转成车体 4 角点多边形，与 2048 个基元比最近距离（`_match_agent_token`） |
| **decode** token → 轨迹 | **推理时还原** | 查表得 6 步基元，逐段 `transform_to_global` 自回归拼接（`decode_token_ids_to_trajectory`） |

**round-trip 实测**（200 个 navtrain 样本，`GT轨迹 → token → 解码还原`）：

```
平均误差 0.048 m   中位 0.029 m   误差 >1m 的样本: 0/200
```

**编解码本身没问题。** 这 5 cm 是 codebook 分辨率决定的**下限** —— 再完美的模型也超不过。
（这个检验很有用：排查训练问题时，它能干净地排除"标签有问题"这条路。）

### 5.5.3 loss 就是 action token 上的交叉熵

```
GT trajectory ──codebook──► gt_idx ──► labels
                                          ↕  CrossEntropy
                                       model logits
```
...
**① CE 是在完整词表上算的，不是 2048 类。** `lm_head` 输出维度 **153713**（151665 原词表 + 2048 action token）。
所以模型先得学会"这个位置该输出 action 区间的 token"，再学"具体是哪个"。前者几步就会，难点在后者。
参照：完全随机猜 2048 类的 loss = `ln(2048) = 7.62`。

**② 相邻编号 ≈ 相似动作。** codebook 按运动基元聚类得来，编号相近的几何上也相近。
所以 **top-1 这个指标偏严格** ——"猜到隔壁"和"猜错十万八千里"在它眼里一样。
判断模型好坏时应**同时看 ADE** 交叉印证（实测：坏 ckpt ADE 2.65m，发布 ckpt 0.23m）。

### 5.5.4 训练/验证是 teacher-forcing，评测才是自回归

`validation_step` 里 `self.autovla(batch)` 是**一次前向 + labels**，而 `batch['input_ids']` 里
**已经包含了正确答案的 10 个 action token**：

```
位置:    951      952      953      954      955  ...
输入:  <act_30> <act_226> <act_806> <act_386>  ?
       └──────── 全是 GT，模型直接读得到 ────────┘
```

预测第 k 个 action 时，前 k−1 个**正确答案**已在输入里 —— 这就是 teacher-forcing。

**为什么这么设计**：速度。TF 一次前向就能并行算完 10 个位置；自回归要串行解码 10 次，
实测约 2.3 s/样本，2000 个 val 样本要 >1 小时，**比训练本身还慢**。

**代价**：TF 无误差累积，系统性高估真实能力。实测换算比例：

| | teacher-forcing | 自回归 |
|---|---|---|
| 坏 ckpt (bf16) | 16.3% | **7.7%** |
| 发布 RFT ckpt | — | **77.7%**（navtest，真正 held-out） |

**所以 `val_action_top1` 只能看自己的趋势，不能跟 77.7% 横向比。**
真实数字要靠 PDMS 评测（`AutoVLA.predict()` 里调 `vlm.generate()`，是自回归）。
